...
app.get(config.uploadAppPath, (req, res) => {
  ...
  if (sid) {
    try {
      ...
      // If bucket exists, has files and is locked, show download UI
      // directly at the same URL (/main?sid=...), without redirect.
      if (bucket && bucket.length > 0 && db.isLocked(sid)) {
        return res.send(downloadPage({ ...pugVars, lang: req.translations }));
      }
    } catch (e) {
      console.error(e);
    }
...
// Create upload bucket (pre-registers sid, only these can receive uploads)
const fsp = require('fs-promise');
app.post(`${ config.uploadAppPath }create-upload-bucket`, async (req, res) => {
  if (config.uploadPass) {
    const bfTimeout = 200;
    if (!req.get('x-passwd')) {
      setTimeout(() => res.status(401).json({ error: 'Unauthorized' }), bfTimeout);
      return;
    }
    if (req.get('x-passwd') !== config.uploadPass) {
      setTimeout(() => res.status(403).json({ error: 'Forbidden' }), bfTimeout);
      return;
    }
  }
  const sid = randomUUID().replace(/-/g, '').slice(0, 12);
  const bucketDir = path.join(config.uploadDir, sid);
  const markerPath = path.join(bucketDir, '.registered');
  try {
    await fsp.ensureDir(bucketDir);
    await fsp.writeFile(markerPath, '', 'utf8');
  } catch (e) {
    debug('create-upload-bucket failed: %s', e.message);
    return res.status(500).json({ error: 'Failed to create bucket' });
  }
  res.json({ sid });
});

// Check if upload bucket exists (used for validating ?sid= links)
app.get(`${ config.uploadAppPath }check-upload-bucket/:sid`, async (req, res) => {
  const { sid } = req.params;
  // simple format validation: 12 hex chars
  if (!/^[a-f0-9]{12}$/.test(sid)) {
    return res.status(400).json({ ok: false, error: 'Invalid sid format' });
  }
  const bucketDir = path.join(config.uploadDir, sid);
  const markerPath = path.join(bucketDir, '.registered');
  try {
    ...
    debug('check-upload-bucket miss for %s: %s', sid, e.message);
  ...
  async function(req, res, next) {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        // Only pre-registered buckets can receive uploads (prevents random ?sid= in URL)
        //const markerPath = path.join(config.uploadDir, meta.sid, '.registered');
        //try {
          //await fsp.access(markerPath);
        //} catch {
          //return res.status(403).json({ message: 'Invalid upload link. Bucket not registered.' });
        //}
